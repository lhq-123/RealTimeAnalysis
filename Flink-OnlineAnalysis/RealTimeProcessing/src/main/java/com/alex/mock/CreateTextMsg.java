 ...
 * @Description 创建模拟订单数据，输出到txt文件中;
 */
public class CreateTextMsg {
    ...
    public static void main(String[] args) throws Exception {
        ...
        date = DateUtils.parseDate(day + " 00:00:00", "yyyy-MM-dd HH:mm:ss");
        ...
        for (;i<size;i++){
            ...
            if (goodsType.equals("数码")){
                price = RandomUtils.nextDouble(500.00, 10000.00);
                num = RandomUtils.nextInt(1, 2);
            }else if (goodsType.equals("家居")){
                price = RandomUtils.nextDouble(500.00, 8000.00);
                num = RandomUtils.nextInt(1, 2);
            }else if (goodsType.equals("运动")){
                price = RandomUtils.nextDouble(300.00, 5000.00);
                ...
                price = RandomUtils.nextDouble(200.00, 2000.00);
                num = RandomUtils.nextInt(1, 4);
            }else {
                price = RandomUtils.nextDouble(10.00, 1000.00);
            ...
            date = DateUtils.addMilliseconds(date, RandomUtils.nextInt(25, 16000));
            String time = DateFormatUtils.format(date,"HH:mm:ss");
            Long orderTimeSeries = date.getTime();

            String status = statuss[RandomUtils.nextInt(0, statuss.length)];
            String phone = String.format("13%s%09d", (i+1)%9, i);
            //用户名称、用户性别、商品名称、商品类型、生产商、下单时间、单价、数量、总价、订单状态、 收货地址、联系方式
            Order order = new Order(orderId, userName, gender, goods, goodsType, brand, day +" "+time, orderTimeSeries , price, num, totalPrice, status, address, Long.parseLong(phone));
            String orderJson = new Gson().toJson(order);
            System.out.println(orderJson);
            orderList.add(orderJson);
        }
        //写入文件
        FileUtils.writeLines(new File("D://test//order.txt"), "UTF-8", orderList, false);
    }
